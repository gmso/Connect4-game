...
def test_BoardValidator_construction():
    ...
    assert(validator.game_won(board_empty.columns) == False)
...
def test_BoardValidator_game_won_with_columns():
    ...
    assert(validator.game_won(board_won_reds.columns) == True)

    board_won_yellows = BoardMaker.make_board_won_with_column_of_yellows()
    assert(validator.game_won(board_won_yellows.columns) == True)
...
def test_BoardValidator_game_won_with_diagonals():
    ...
    board_won_reds = BoardMaker.make_board_won_with_diagonal_downwards_of_reds()
    assert(validator.game_won(board_won_reds.columns) == True)

    board_won_yellows = BoardMaker.make_board_won_with_diagonal_upwards_of_yellows()
    assert(validator.game_won(board_won_yellows.columns) == True)
